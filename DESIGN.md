...
- **Current page** (`nav a[aria-current="page"]`): inverts to a white chip carrying the Poké Ball itself at 18px — `PokeBall.astro` in its `neutral` variant, the same mark as the brand plate and the footer, whose ink ring is what keeps a white shell legible on a white chip. It has a lifting shadow, and it deliberately does **not** pulse; only the live map marker animates. *Owner deviation, 2026-09-05: this was a 10px red disc in an ink ring drawn in `::before` — the You-Are-Here marker restated on the bar as a sign. It read as a dot rather than as the brand. The mark is in the markup now because a pseudo-element cannot hold an SVG we already own.*
- **Go chip** (`.nav-highlight`): the GO mark, unfilled, at 30px tall. *Owner deviation, 2026-09-05: this was the one black nav chip with the word "Go" in it. No fill sits behind the mark — it is already a dark disc with a silver ring around a starfield, so an ink chip put a dark shape behind a dark shape and cost the ring its edge; on the red bar the ring is what separates it. Hover and focus come from `nav a`, and the current-page state is the shared white chip rather than a private one.* The art is `/art/go-mark.webp`, cut and circle-masked from `/art/logo-txk-classic.webp` — **our own wordmark, which already ships and already carries this mark on the home page**, not from `art-src/Pokemon_Go.svg.webp`, which stays out of the build under `vault/Attribution Obligations.md` §3. `alt="Go"` keeps the link named without the image; 30px is the floor at which the letterforms and the ring still read, measured at 22px first.
...
- **Account menu:** the signed-in control opens rows in a fixed order — **Admin** (to `/admin`) first, for anyone the `/admin` gate admits; **Sign out**; then **Use a different Discord account** and **Delete my account**, for Discord accounts only. A standalone admin identity (`admin:<name>`, no Discord account behind it) sees just Admin and Sign out. The server decides both through `/api/me.json` (`canAdmin`, `standaloneAdmin`); the inline script compares no roles. Every row is the same element with the same style, and the floating panel keeps its 260px minimum whichever rows it holds, so it is one size for everyone (added 2026-09-23).
